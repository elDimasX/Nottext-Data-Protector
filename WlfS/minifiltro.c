...
)
{
	...
	*/
	...
	__try
	{
		...
		if (BloquearArquivo == TRUE && ProtecaoHabilitada == TRUE)
		{
			// Se nao for para ignorar e a proteção estiver habilitada
			if (!ProcessoIgnorarPeloUserMode(Processo) && Processo != ProcessoNottextBackup)
			{
...
)
{
	...
	__try
	{
		...
		// Se for para proteger contra modificação
		if (Bloquear == TRUE && ProtecaoHabilitada == TRUE)
		{
			PEPROCESS Processo = IoThreadToProcess(Data->Thread);
